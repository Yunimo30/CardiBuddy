...
export default function GpaEngine() {
  ...
  const handleUpdateComponent = (id, field, value) => {
    const updatedSchema = currentSchema.map(comp => comp.id === id ? { ...comp, [field]: value } : comp);
  ...
  const gradedWeight = currentSchema.reduce((sum, comp) => isGraded(comp) ? sum + (Number(comp.weight) || 0) : sum, 0);
  const earnedPoints = currentSchema.reduce((sum, comp) => isGraded(comp) ? sum + ((Number(comp.earned) / Number(comp.max)) * (Number(comp.weight) || 0)) : sum, 0);
  ...
  return (
    <div className="max-w-6xl mx-auto h-full flex flex-col space-y-2 sm:space-y-6">
      ...
      <div>
        <h1 className="text-xl sm:text-2xl font-extrabold text-white tracking-tight">GPA Engine</h1>
        <p className="text-[10px] sm:text-sm font-medium text-zinc-400 mt-0.5 sm:mt-1">Design grading schemas and track targets</p>
      </div>

      <div className="flex-1 flex flex-col md:flex-row gap-3 sm:gap-6 overflow-hidden pb-2 sm:pb-6">
        
        {/* Horizontal Pill Bar for Mobile */}
        ...
        <div className="flex-1 flex flex-col bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-lg">
          {activeCourseData ? (
            <>
              ...
              <div className="p-3 sm:p-6 border-b border-zinc-800 flex justify-between items-center bg-zinc-950/20 shrink-0">
                <div className="overflow-hidden">
                  ...
                  <p className="hidden sm:block text-xs sm:text-sm text-zinc-400 font-medium">Grading Schema</p>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-3 bg-zinc-950 border border-zinc-800 rounded-lg p-1 sm:p-2 shadow-inner shrink-0">
                  <Target size={14} className="text-red-500 ml-1" />
                  <span className="text-[9px] sm:text-xs font-bold text-zinc-400 uppercase tracking-widest hidden sm:inline">Target %</span>
                  <input 
                    type="number" 
                    value={currentTargetUI} 
                    onChange={(e) => handleUpdateTarget(e.target.value)}
                    className="w-10 sm:w-16 bg-zinc-900 border border-zinc-700 rounded text-white text-center font-bold text-xs sm:text-sm py-1 outline-none focus:border-red-500 transition-colors"
                  />
                </div>
              </div>

              {/* Tighter Form Elements */}
              <div className="flex-1 overflow-y-auto p-2 sm:p-6 custom-scrollbar bg-zinc-900/50 flex flex-col gap-2 sm:gap-4">
                
                {totalWeight !== 100 && (
                  <div className="flex items-center gap-2 text-[9px] sm:text-xs font-bold text-amber-400 bg-amber-500/10 p-2 sm:p-3 rounded-lg border border-amber-500/20">
                    <AlertTriangle size={14} className="shrink-0" /> 
                    Weights sum to {totalWeight}%, not 100%.
                  </div>
                )}

                {currentSchema.map((comp) => (
                  <div key={comp.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 bg-zinc-950 border border-zinc-800 p-2 sm:p-4 rounded-xl shadow-sm hover:border-zinc-700 transition-colors relative pr-8 sm:pr-4">
                    
                    <div className="w-full sm:flex-1">
                      <label className="text-[8px] sm:text-[9px] font-bold text-zinc-500 uppercase tracking-widest block mb-0.5">Category</label>
                      <input 
                        type="text" 
                        value={comp.name || ""} 
                        onChange={(e) => handleUpdateComponent(comp.id, "name", e.target.value)}
                        className="w-full bg-zinc-900 border border-zinc-800 rounded-md p-1.5 sm:p-2 text-xs sm:text-sm text-white focus:border-zinc-600 outline-none" 
                      />
                    </div>
                    
                    <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                      <div className="w-16 sm:w-20">
                        <label className="text-[8px] sm:text-[9px] font-bold text-zinc-500 uppercase tracking-widest block mb-0.5">Weight %</label>
                        <input 
                          type="number" 
                          value={comp.weight || ""} 
                          onChange={(e) => handleUpdateComponent(comp.id, "weight", e.target.value)}
                          className="w-full bg-zinc-900 border border-zinc-800 rounded-md p-1.5 sm:p-2 text-xs sm:text-sm text-white focus:border-zinc-600 outline-none text-center font-mono" 
                        />
                      </div>

                      <div className="flex-1 sm:w-36">
                        <label className="text-[8px] sm:text-[9px] font-bold text-zinc-500 uppercase tracking-widest block mb-0.5 text-center">Score (Earned/Max)</label>
                        <div className="flex items-center gap-1">
                          <input 
                            ...
                            className={`w-full bg-zinc-900 border border-zinc-800 rounded-md p-1.5 sm:p-2 text-xs sm:text-sm font-bold text-center outline-none transition-colors ${comp.earned && comp.earned !== "" ? "text-emerald-400 border-emerald-500/30" : "text-zinc-500"}`} 
                          />
                          ...
                          <input 
                            ...
                            className="w-full bg-zinc-900 border border-zinc-800 rounded-md p-1.5 sm:p-2 text-xs sm:text-sm font-bold text-zinc-300 text-center outline-none focus:border-zinc-600 transition-colors" 
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    <button 
                      ...
                      className="absolute top-2 right-2 sm:static sm:p-2 text-zinc-600 hover:text-red-500 transition-colors shrink-0"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
                ...
                <button 
                  ...
                  className="flex items-center justify-center gap-2 border border-dashed border-zinc-700 hover:border-zinc-500 text-zinc-400 hover:text-white p-2 sm:p-3 rounded-xl transition-colors font-bold text-xs sm:text-sm w-full"
                >
                  <Plus size={14} /> Add Category
                </button>
              </div>

              {/* Compressed Footer Grid */}
              <div className="border-t border-zinc-800 bg-zinc-950 p-2 sm:p-6 shrink-0 grid grid-cols-3 gap-1.5 sm:gap-4">
                
                <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-1.5 sm:p-3 text-center flex flex-col justify-center">
                  <p className="text-[7px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-0.5 leading-tight">Graded</p>
                  <p className="text-sm sm:text-xl font-extrabold text-white font-mono leading-none">{gradedWeight}%</p>
                </div>
                
                <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-1.5 sm:p-3 text-center flex flex-col justify-center">
                  <p className="text-[7px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-0.5 leading-tight">Standing</p>
                ...
                <div className={`rounded-lg p-1.5 sm:p-3 text-center shadow-md border flex flex-col justify-center ${
                  ...
                  <p className="text-[7px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-0.5 leading-tight">Needed</p>
            ...
            <div className="h-full flex flex-col items-center justify-center text-zinc-600 font-medium gap-2 sm:gap-4 opacity-60 p-4 text-center">
               <GraduationCap size={40} className="sm:w-12 sm:h-12" />
               <p className="text-xs sm:text-base">Select a course to design its schema.</p>
            </div>
          )}
        </div>
      </div>
